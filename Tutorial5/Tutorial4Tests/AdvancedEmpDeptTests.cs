...
public class AdvancedEmpDeptTests
{
    ...
    {
        var emps = Database.GetEmps();
        var result = emps.All(e => e.Comm > 400);
         Assert.True(result);
    }
    ...
    public void ShouldReturnEmployeeManagerPairs()
    {
        ...
        var result = emps.Join(emps, emp => emp.Mgr, manager => manager.EmpNo, (emp, manager) => new { Employee = emp.EName, Manager = manager.EName }).ToList();
    ...
    public void ShouldJoinEmpDeptSalgrade()
    {
        ...
        var result = emps.Join(depts, emp => emp.DeptNo, dept => dept.DeptNo, (emp, dept) => new { emp, dept }).SelectMany(pair => grades.Where(grade => pair.emp.Sal <= grade.Hisal && pair.emp.Sal >= grade.Losal).Select(grade => new { EName = pair.emp.EName, DName = pair.dept.DName, Grade = grade.Grade})).ToList();
