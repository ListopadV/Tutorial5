...
public class EmpDeptSalgradeTests
{
    ...
    {
        var emps = Database.GetEmps();
        var result = emps.Select(row => new { row.EName, row.Sal }).ToList();
         Assert.All(result, r =>
         {
    ...
    {
        var emps = Database.GetEmps();
        var result = emps.Select(e => new { EName = e.EName, Comm = e.Comm }).Where(e => e.Comm != null);
    ...
    public void ShouldMatchEmployeeToSalaryGrade()
    {
        ...
        var result = emps.SelectMany(emp => grades.Where(grade => emp.Sal <= grade.Hisal && emp.Sal >= grade.Losal).Select(grade => new { emp.EName, grade.Grade })).ToList();
    ...
    public void ShouldReturnEmployeesEarningMoreThanDeptAverage()
    {
        ...
        var result = emps.Where(e => e.Sal > emps.Where(emp => emp.DeptNo == e.DeptNo).Average(emp => emp.Sal)).ToString();
